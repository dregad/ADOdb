	...
	*/
    public function runUpdateSrcFn($srcdb, $table, $fldoffsets, $row, $where, $mode, $dest_insertid = null, $lastUpdateFld = '')
    {
        ...
        if (is_array($fn)) {
            ...
            $fn($srcdb, $table, $row, $where, $bindarr, $mode, $dest_insertid);
        }
        ...
        foreach ($types as $name => $t) {
            ...
            if ($this->copyTableDefaults) {
                if (isset($t->default_value)) {
                    $v = $t->default_value;
                    if ($mt == 'C' or $mt == 'X') {
                        $v = $this->connDest->qstr($v); // might not work as this could be function
                    }                    $s .= ' DEFAULT '.$v;
                }
            }
	...
	*/
    ...
    public function replicateData(
        ...
        if (is_array($where)) {
            ...
            $this->_clearcache();
        if (is_string($uniqflds) && strlen($uniqflds)) {
            $uniqflds = array($uniqflds);
        }
        if (!$desttable) {
            $desttable = $table;
        }
            ...
            foreach ($types as $name => $t) {
                ...
                // handle quotes
                if ($name2 && $name2[0] == '"' && $name2[strlen($name2)-1] == '"') {
                    $name22 = substr($name2, 1, strlen($name2)-2);
                } elseif ($name2 && $name2[0] == '`' && $name2[strlen($name2)-1] == '`') {
                    $name22 = substr($name2, 1, strlen($name2)-2);
                } else {
                    $name22 = $name2;
                }
                ...
                if (!isset($dtypes[($name22)]) || !$name2) {
                    if ($this->debug) {
                        echo " Skipping $name ==> $name2 as not in destination $desttable<br>";
                    }
                    continue;
                }
                ...
                if ($this->datesAreTimeStamps && $mt == 'D') {
                    $mt = 'T';
                }
                if ($mt == 'D') {
                    $fldval = $dest->DBDate($fldval);
                } elseif ($mt == 'T') {
                    $fldval = $dest->DBTimeStamp($fldval);
                }
                $ufld = strtoupper($fld);
                ...
                if ($this->debug) {
                    echo " field=$fld type=$mt fldval=$fldval<br>";
                }
                ...
                if (!isset($uniq[$ufld])) {
                    ...
                    if ($mt == 'D') {
                        $p = $dest->DBDate($p, true);
                    } elseif ($mt == 'T') {
                        $p = $dest->DBTimeStamp($p, true);
                    }
                    ...
                    if (!empty($srcuniqflds)) {
                        $srcwheref[] = $srcuniqflds[$uniq[$ufld]];
                    }
            ...
            foreach ($wheref as $uu => $fld) {
                ...
                if (!empty($srcuniqflds)) {
                    if ($uu > 1) {
                        die("Only one primary key for srcuniqflds allowed currently");
                    }
            ...
            $seltable = $table;
            if ($this->readUncommitted && strpos($src->databaseType, 'mssql')) {
...
';
            ...
            if (!$this->execute) {
                ...
                while ($origrow = $rs->FetchRow()) {
                    ...
                    if ($fn) {
                        if (!$fn($desttable, $origrow, $deleteFirst, $this, $selflds)) {
                            continue;
                        }
                    }
                    $doinsert = true;
                    ...
                    if (!$onlyInsert) {
                        ...
                        if ($upderr || $dest->Affected_Rows() == 0) {
                            ...
                            if (!empty($uniqflds)) {
                                $this->RunUpdateSrcFn($src2, $table, $fldoffsets, $origrow, $srcwheress, 'UPD', null, $lastUpdateFld);
                            }
                            $upd += 1;
                        }
                    }
                    ...
                    if ($doinsert) {
                        ...
                        if (! $dest->Execute($sa['INS'], $row)) {
                            ...
                            if ($dest->dataProvider == 'oci8') {
                                if ($this->oracleSequence) {
                                    $lastid = $dest->GetOne("select ".$this->oracleSequence.".currVal from dual");
                                } else {
                                    $lastid = 'null';
                                }
                            } else {
            ...
            if ($cnt != $ins + $upd) {
                echo "<p>ERROR: $cnt != INS $ins + UPD $upd</p>";
            }
            $src->setFetchMode($savemode);
	...
	";
            ...
            } elseif (strpos($src->databaseType, 'oci') !== false) {

                if (strlen($srcTable)>22) {
                    $tableidx = substr($srcTable, 0, 16).substr(crc32($srcTable), 6);
                } else {
                    $tableidx = $srcTable;
                }
...
";
	...
	*/
    ...
    public function merge(
        ...
        if ($orderBy) {
            ...
            $where .= ' ORDER BY '.$srcUpdateDateFld;
        }
        ...
        if ($srcCopyFlagFld) {
            $set[] = "$srcCopyFlagFld = ".$src->qstr($flagvals[2]);
        }
        if ($srcCopyDateFld) {
            $set[]= "$srcCopyDateFld = ".$src->sysTimeStamp;
        }
        if ($set) {
            $this->updateSrcFn = array(implode(', ', $set));
        } else {
            $this->updateSrcFn = '';
        }
            ...
            if ($this->compat && $this->compat == 1.0) {
                $srcUpdateDateFld = '';
            }
	...
	*/
    ...
    protected function _doerr($reason, $selflds)
    {
        $fn = $this->errHandler;
        if ($fn) {
            $fn($this, $reason, $selflds); // set $this->neverAbort to true or false as required inside $fn
        }
    }
}
